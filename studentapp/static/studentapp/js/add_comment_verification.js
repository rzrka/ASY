...
(function() {
    ...
    $('#save_comment').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                var $element = $('*[data-criterion="' + teleworkcriterion + '"]').find('.comment_block');
                if (!$element.length) {
                    $element = $('*[data-criterion="' + teleworkcriterion + '"]').find('span:nth-child(3) a[data-iframe="add_comment"]').before('<div class="comment_block"><div><b>Примечания студента:</b></div></div>');
                    $element = $('*[data-criterion="' + teleworkcriterion + '"]').find('.comment_block');
                }
                $element
                    .append(' <div data-comment_id="' + comment_id + '" class="colored_text_comment comment">' + text + '<a class="comment_delete"></a></div>');
            },
            error: function(data) {
                console.log(data);
            }
    ...
    $('div[data-criterion]').on('click', 'a.comment_delete', function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.success) {
                    var parent = div_parent.closest('div.comment_block');
                    div_parent.remove();
